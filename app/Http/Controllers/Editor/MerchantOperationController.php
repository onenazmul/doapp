...
$merchantInfo = Merchant::find($id);
        ...
        $totaldue = DB::table('parcels')
            ->join('merchants', 'merchants.id','=','parcels.merchantId')
            ->where('parcels.merchantId', $id)->where('parcels.status',4)->where('parcels.status','!=',9)
            ->sum('parcels.merchantDue');
        ...
        $parcels = DB::table('parcels')
            ...
            ->get();
    	return view('backEnd.merchant.view',compact('merchantInfo','deliverycharge','parcel','totalamount','totaldue','parcels','collectedAmount','merchantDue','marcentamount','merchantPaid'));
    }
