...
class ParcelManageController extends Controller
{
 ...
 public function manage(){
    ...
    $paid =Parcel::where('status',4)->whereBetween('created_at', [$request->startDate, $request->endDate])->sum('merchantPaid');
    $unpaid =Parcel::where('status',4)->whereBetween('created_at', [$request->startDate, $request->endDate])->sum('merchantDue');
...
// dd($parcelprice);
    $deliveryCharge =Parcel::whereBetween('created_at', [$request->startDate, $request->endDate])->where('status','!=',9)->sum('deliveryCharge');
    $deliveryCharged= Parcel::whereBetween('created_at', [$request->startDate, $request->endDate])->where('status',4)->sum('deliveryCharge');

    $codCharge= Parcel::whereBetween('created_at', [$request->startDate, $request->endDate])->where('status',4)->sum('codCharge');
    ...
    $paid =Parcel::where('status',4)->where('merchantId',$request->mid)->whereBetween('created_at', [$request->startDate, $request->endDate])->sum('merchantPaid');
    $unpaid =Parcel::where('status',4)->where('merchantId',$request->mid)->whereBetween('created_at', [$request->startDate, $request->endDate])->sum('merchantDue');
     $deliveryCharge =Parcel::where('merchantId',$request->mid)->where('status','!=',9)->whereBetween('created_at', [$request->startDate, $request->endDate])->sum('deliveryCharge');
...
// dd($parcelprice);
    $deliveryCharged= $parcelprice =Parcel::where('merchantId',$request->mid)->whereBetween('created_at', [$request->startDate, $request->endDate])->where('status',4)->sum('deliveryCharge');

    $codCharge= $parcelprice =Parcel::where('merchantId',$request->mid)->whereBetween('created_at', [$request->startDate, $request->endDate])->where('status',4)->sum('codCharge');
...
// dd($parcelprice);
    ...
    $Collectedamount =Parcel::where('merchantId',$request->mid)->where('status',4)->whereBetween('created_at', [$request->startDate, $request->endDate])->sum('cod');
 $deliveryCharged = Parcel::where('merchantId',$request->mid)->whereBetween('created_at', [$request->startDate, $request->endDate])->where('status',4)->sum('deliveryCharge');
    $parcelcount =Parcel::where('merchantId',$request->mid)->whereBetween('created_at', [$request->startDate, $request->endDate])->count();
    }
...
$all=Parcel::all()->count();
     ...
     $store_parcel->trackingCode  = 99999+$all+1;
     ...
     $this->validate($request,[
        ...
        //   $codcharge= $initialcodcharge->codcharge;
         ...
         $update_parcel->agentId = $hub->hub_id;
          ...
          $update_parcel->user = Auth::user()->name;
