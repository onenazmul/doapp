...
<head>
  ...
  /*}*/
      ...
      @media print {
        ...
        .main-footer, .print-hide{display: none;}
      }
                    ...
                    <div class="border-top border-bottom mb-2 pt-1 pb-1" style="background: #f8f9fa">
                        <div class="row">
                            <div class="col-7">
                                ...
                                <strong>Merchant Name</strong>: {{$merchantInfo->firstName}}<br>
                                <strong>Mobile Number</strong>: {{$merchantInfo->companyName}}
                            </div>
                            <div class="col-5">
                                <div class="row">
                                    <div class="col-12 text-end">
                                        ...
                                        <strong>Prepared By</strong>: {{$invoiceInfo->user}}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-12">
                        <div class="table-responsive">
                           ...
                           ?> {{$value->partial_pay}}</td>
                                    ...
                                    @php
                                    $payment += (int)($value->cod-($value->deliveryCharge+$value->codCharge))-$partial;
                                    $subtotal+=(int)($value->cod-($value->deliveryCharge+$value->codCharge))-$partial;
                                    $chage+=(int)$value->deliveryCharge+$value->codCharge;
                                    $partial+=(int)$value->partial_pay;
                                    $total+=(int)$value->cod;
                                    @endphp
                                    @endforeach
                                    <tr>
    						      <td class="text-end fw-bold" colspan="3">Invoice Payment Total:</td>
    						      
    						      <td class="text-end fw-bolder">{{ $total}}</td>
    						      <td class="text-end fw-bolder">{{ $partial}}</td>
    						      <td class="text-end fw-bolder">{{ $chage}}</td>
    						      <td class="text-end fw-bolder">{{ $subtotal}}</td>
    						      <td class="text-end fw-bolder">{{ $payment}}</td>
    						    </tr>
                                </tbody>
                            </table>
                        </div>
